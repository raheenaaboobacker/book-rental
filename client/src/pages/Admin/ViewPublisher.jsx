...
export default function ViewPublisher() {
  const navigate=useNavigate()
    ...
    useEffect(() => {
       ...
       const deleteuser=(id)=>{
        console.log(id);
        axios.delete(`http://localhost:5000/admin/delete-publisher/${id}`)
         .then(response=>{
          if(response.data.success===true){
           swal(response.data.message)
           setMessage(!message)
  ...
  <div className="app-title">
    <div>
      <h1><i className="fa fa-users" /> Publisher</h1>
      {/* <p>A free and open source Bootstrap 4 admin template</p> */}
    </div>
    <ul className="app-breadcrumb breadcrumb">
      <li className="breadcrumb-item"><i className="fa fa-home fa-lg" /></li>
      <li className="breadcrumb-item"><a href="#">Publisher</a></li>
    </ul>
  </div>
  <div className='row'>
  {volunteer&&volunteer.map(item=>(
    ...
    <div className="card card-profile">
      ...
      <div className="card-body">
        ...
        <a onClick={()=>deleteuser(item._id)} className="btn btn-primary btn-round">Delete</a><br/>
